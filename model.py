...
def get_tool_call_from_model(
    ...
    prompt = f"""
你是一个 tool_agent 链路中的工具选择模块。

上层 router 已经判断：这个用户输入需要进入 tool_agent。
所以你不需要判断用户输入是不是普通聊天，也不需要回答用户问题。

你的唯一任务：根据用户原始任务、当前轮次、消息轨迹和可用工具，选择“当前这一步”应该执行的一个工具。

如果所有需要工具完成的子任务都已经有 role="tool" 的成功结果，或者当前没有任何可用工具能处理剩余子任务，才返回 need_tool=false。
...
工具能力询问规则：
1. 如果用户只是询问“你有什么工具、你能使用哪些工具、你有没有联网工具、你能不能联网搜索、你是不是有 web_search”，不要调用任何工具。
2. 不要为了证明工具存在而调用工具。
3. 不要使用空 query 调用 web_search。
4. 只有当用户明确给出搜索对象、关键词、网址，或者要求获取外部信息时，才调用 web_search。
5. 如果这类工具能力询问意外进入 tool_agent，返回 need_tool=false，并说明“这是工具能力询问，不需要执行具体工具”。
...
4. 不要把普通聊天、概念解释、记忆查询当作你的职责；这些输入正常情况下不会进入 tool_agent。
5. 如果它们意外进入 tool_agent，返回 need_tool=false，并说明“当前 tool_agent 没有可执行工具任务”。
...
6. 不要输出 JSON。
7. 直接回答用户。
8. 如果用户只是询问工具能力、是否有某个工具、能不能联网搜索，用自然语言直接回答工具能力，不要使用“已完成 / 未完成 / 原因”的任务报告格式。
9. 如果没有具体搜索关键词，不要把空查询或演示性工具调用包装成已完成搜索任务，只说明“不需要实际调用 web_search”。
...
2. 如果用户只是询问“你有什么工具、你能使用哪些工具、你有没有联网工具、你能不能联网搜索、你是不是有 web_search”，这是工具能力说明问题，选择 llm_only，不要选择 tool_agent。
3. 只有当用户明确要求执行外部动作、搜索具体关键词、读取文件、查询系统状态、调用 API、获取真实外部信息时，才选择 tool_agent。
4. 如果任务不需要工具，但明显包含多个连续 LLM 步骤，选择 llm_workflow。
5. 其他普通单步任务，选择 llm_only。
...
"""
    model_text = ask_deepseek_model(prompt, model_name = 'deepseek-chat')
    router_result = json.loads(model_text)
